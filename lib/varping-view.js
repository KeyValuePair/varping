...
const compare_substr = function(pattern, target) {
    ...
    for (let i = 0; i < lowerPattern.length; i++) {
        if (lowerPattern[i] !== lowerTarget[i]) {
            return false;
        }
    }
...
const strong_compare_substr = function(pattern, target) {
...
let getRelatedStrings = function(pattern, targets) {
    ...
    for (let i = 0; i < targets.length; i++) {
        ...
        if (strong_compare_substr(pattern, target)) {
            result.push(targets.splice(i--,1));
        }
    }
...
export default class VarpingView {
  ...
  constructor(serializedState) {
    ...
    inputArea.setAttribute("onblur", "this.focus()");
    ...
    inputArea.oninput = function() {
        ...
        let targets = ['Kasdf', 'Kanan', 'Kotnda', 'Asdf', 'Sdfw', 'sASDf'];
